...
import com.reply.tyreshop.test.test.groovy.webservicetests.markers.CollectOutputFromTest
...
class CatalogTests extends BaseWSTest {
	...
	void testGetCatalogsXML() {
		def con = testUtil.getSecureConnection('/catalogs', 'GET', 'XML')
		...
		//OPTIONS CATEGORIES
		response = testUtil.verifiedXMLSlurper(testUtil.getSecureConnection('/catalogs?options=CATEGORIES', 'GET', 'XML'))
		...
		response = testUtil.verifiedXMLSlurper(testUtil.getSecureConnection('/catalogs?options=PRODUCTS,CATEGORIES', 'GET', 'XML'))
		...
		response = testUtil.verifiedXMLSlurper(testUtil.getSecureConnection('/catalogs?options=CATEGORIES,SUBCATEGORIES', 'GET', 'XML'))
		onlineCatalog = onlineCatalog = response.catalog[0].catalogVersions.catalogVersion.find({it.id == 'Online'})
		assert onlineCatalog != null
	...
	void testGetCategoriesXML() {
		def con = testUtil.getSecureConnection('/catalogs/wsTestProductCatalog/Online/categories/brands')
	...
	void testGetCatalogsJSON() {
		def con = testUtil.getSecureConnection('/catalogs', 'GET', 'JSON')
		...
		//OPTIONS CATEGORIES
		response = testUtil.verifiedJSONSlurper(testUtil.getSecureConnection('/catalogs?options=CATEGORIES', 'GET', 'JSON'))
		//TODO catalogVVVersion
		...
		//OPTIONS PRODUCTS (requires categories)
		response = testUtil.verifiedJSONSlurper(testUtil.getSecureConnection('/catalogs?options=PRODUCTS,CATEGORIES', 'GET', 'JSON'))
		...
		//OPTIONS SUBCATEGORIES (requires categories)
		response = testUtil.verifiedJSONSlurper(testUtil.getSecureConnection('/catalogs?options=CATEGORIES,SUBCATEGORIES', 'GET', 'JSON'))
		onlineCatalog = response.catalogs[0].catalogVersions.find{ version -> version.id == 'Online'}
		assert onlineCatalog != null
	...
	void testGetCategoriesJSON() {
		def con = testUtil.getSecureConnection('/catalogs/wsTestProductCatalog/Online/categories/brands', 'GET', 'JSON')
