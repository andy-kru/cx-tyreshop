 ...
 *
 */
package com.reply.tyreshop.test.test.groovy.webservicetests.v2.spock.carts
...
class CartResourceTest extends AbstractCartTest {
	...
	def "Registered and authorized customer creates cart, add one product and checks for tax details on cart: #format"(){
		...
		with(response){
			...
			if(!data.net.toBoolean()) data.totalTax.value.toDouble() > 0.0 // when net is false, tax should be > 0.0
			if(data.net.toBoolean()) data.totalTax.value.toDouble() == 0.0 // when net is true, tax should be 0.0
		}
