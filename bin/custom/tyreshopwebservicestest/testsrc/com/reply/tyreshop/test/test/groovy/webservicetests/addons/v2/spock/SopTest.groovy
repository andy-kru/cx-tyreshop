 ...
 */
package com.reply.tyreshop.test.test.groovy.webservicetests.addons.v2.spock
...
import com.reply.tyreshop.test.test.groovy.webservicetests.v2.spock.carts.AbstractCartTest
...
class SopTest extends AbstractCartTest {
	...
	public void ignoreIf(){
		org.junit.Assume.assumeTrue(Config.getBoolean("tyreshopwebservicestest.enableAccTest", false))
	}
	...
	def "Get SOP payment request details : #format"(){
		...
		with(response){
			...
			resp.find{it.key == 'shipTo_shippingMethod' && it.value == getDeliveryModeForEnabledIntTestPar()}
	...
	def "Guest places oder using SOP : #format"(){
		...
		setDeliveryModeForCart(restClient, anonymous, cart.guid, getDeliveryModeForEnabledIntTestPar(), format, getBasePathWithIntegrationSite())
	...
	protected createCustomerWithCartForIntegrationSite(format)
	{
		...
		setDeliveryModeForCart(restClient, customer, cart.code, getDeliveryModeForEnabledIntTestPar(), format, getBasePathWithIntegrationSite())
		return val;
	}
	...
	def "Customer create payment info in extended SOP flow : #format"(){
		...
		RESTClient paymentProviderClient = createRestClientWithoutParsing(sopRequestDetails.postUrl)
	...
	private String getDeliveryModeForEnabledIntTestPar(){
		def enableWsIntegrationTest = Config.getBoolean("tyreshopwebservicestest.enableWsIntegrationTest", false);
		return enableWsIntegrationTest ? DELIVERY_STANDARD_NET : DELIVERY_STANDARD
	}
}
