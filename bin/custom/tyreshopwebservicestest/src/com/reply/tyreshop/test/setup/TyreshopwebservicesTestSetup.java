 ...
 */
package com.reply.tyreshop.test.setup;
...
public class TyreshopwebservicesTestSetup extends AbstractSystemSetup
{
	...
	private static final Logger LOG = Logger.getLogger(TyreshopwebservicesTestSetup.class);
	...
	public void createProjectData(final SystemSetupContext context)
	{
		...
		getSetupImpexService().importImpexFile("/tyreshopwebservicestest/import/sampledata/user-orders.impex", true, false);
	...
	{
		return Config.getBoolean("tyreshopwebservicestest.enableWsIntegrationTest", false);
	}
	...
	protected List<String> createContentCatalogNames(final boolean enableWsIntegrationTest)
	{
		...
		return catalogNames.stream().filter(s -> enableWsIntegrationTest || !StringUtils.equals(s, WS_INTEGRATION)).collect(Collectors.toList());
	}
	...
	protected List<String> createStoreNames(final boolean enableWsIntegrationTest)
	{
		...
		return storeNames.stream().filter(s -> enableWsIntegrationTest || !StringUtils.equals(s, WS_INTEGRATION)).collect(Collectors.toList());
	}
	...
	protected void loadOptionalSampleData(final List<String> storeNames)
	{
		...
		{
			storeNames.forEach(store -> getSetupImpexService().importImpexFile(String.format("/tyreshopwebservicestest/import/acceleratorwebservicesaddon/%s/solr.impex", store), true, true));
		}

		if (extensionNames.contains("yacceleratorfulfilmentprocess"))
		{
			getSetupImpexService().importImpexFile("/tyreshopwebservicestest/import/orderprocess/store.impex", true, true);
		}
	}

	public void initializePromotionEngineModule()
	{
		getSetupImpexService().importImpexFile("/tyreshopwebservicestest/import/promotionengine/promotionsmodule.impex", true,
				false);
	}
	...
	protected void loadIntegrationData()
	{
		...
		{
			getSetupImpexService().importImpexFile("/tyreshopwebservicestest/import/integration/cis-integration-data.impex", true);
		}

		if (extensionNames.contains("omssampledata"))
		{
			getSetupImpexService().importImpexFile("/tyreshopwebservicestest/import/integration/oms-integration-data.impex", true);
		}
	}
	...
	protected void loadMultiDimensionalData()
	{
		getSetupImpexService().importImpexFile(
				"/tyreshopwebservicestest/import/sampledata/productCatalogs/wsTestProductCatalog/dimension-categories.impex", true,
				false);
		getSetupImpexService().importImpexFile(
				"/tyreshopwebservicestest/import/sampledata/productCatalogs/wsTestProductCatalog/dimension-products.impex", true,
				false);
		getSetupImpexService().importImpexFile(
				"/tyreshopwebservicestest/import/sampledata/productCatalogs/wsTestProductCatalog/dimension-products-prices.impex",
				true, false);
		getSetupImpexService()
				.importImpexFile(
						"/tyreshopwebservicestest/import/sampledata/productCatalogs/wsTestProductCatalog/dimension-products-stock-levels.impex",
						true, false);
